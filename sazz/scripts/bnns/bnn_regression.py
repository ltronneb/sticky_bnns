...
def build_target_learned_noise(X: Tensor, y: Tensor, *,
    layer_sizes: Sequence[int],
    activation: str = "relu",
    ...
    x_ref, Sigma_inv = find_reference_bnn(
        model.energy, extended_spec.D, model=model, dtype=dtype, device="cpu",
        reference="laplace_diag", n_steps=adam_steps, lr=1e-2,
    )
    ...
    return TorchTarget(
        ...
        meta={"model": model, "spec": extended_spec, "module": module,
              "base_spec": base_spec, "layer_sizes": layer_sizes},
    )
...
def sample_bnn(
    ...
    if noise_std is not None:
        ...
        target = build_target_learned_noise(
            X, y, layer_sizes=layer_sizes, activation=activation,
            prior_std_weight=prior_std_weight, prior_std_bias=prior_std_bias,
            fan_in_scaling=fan_in_scaling,
        )
...
if __name__ == "__main__":
    ...
    samples, target, result = sample_bnn(
        x, y,
        layer_sizes=[1, 50, 1],
        activation="tanh",
        noise_std=0.1,
