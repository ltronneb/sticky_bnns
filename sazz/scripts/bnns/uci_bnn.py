""" The likelihood is an nn.Module evaluated
...
def configs_for(input_dims: dict[str, int]) -> dict[str, BNNConfig]:
    ...
    for name in ("boston", "naval", "energy"):
        if name in input_dims:
            cfgs[name] = BNNConfig(
                layer_sizes=[input_dims[name], *HIDDEN, 1],
                noise_std=0.01 if name == "naval" else 0.3,
...
def make_split(X: np.ndarray, y: np.ndarray, seed: int,
    ...
    return {
        "X_train": torch.tensor((X_tr - x_mean) / x_std),
        "y_train": torch.tensor((y_tr - y_mean) / y_std),
        "X_test":  torch.tensor((X_te - x_mean) / x_std),
        "y_test":  torch.tensor((y_te - y_mean) / y_std),
        "y_std":   float(y_std),
    }
...
def run_split(dataset_name: str, split_id: int, data: dict[str, Any],
    ...
    suffix = "_ln" if cfg.learned_noise else ""
    ...
    for name in samplers:
        ...
        if name == "nuts":
            ...
            t0 = time.perf_counter()
            result = sampler.sample(N=N_SKELETON, diagnostics=True, x0=target.x_ref)
...
def main():
    ...
    cfgs = configs_for({n: X.shape[1] for n, (X, _) in raw.items()
                        if n in uci_to_run})
    if args.learned_noise:
        for cfg in cfgs.values():
            cfg.learned_noise = True

    print(f"\nRunning splits {args.splits} on {uci_to_run} | "
            f"samplers {samplers}")
    for ds in uci_to_run:
        X, y = raw[ds]
