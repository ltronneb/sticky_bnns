...
def build_target(
    ...
    """Build a multiclass classification BNN target.

    The network's last layer must have `n_classes` outputs (the unnormalised
    logits); softmax is applied inside the Categorical likelihood. Swap in a
    different nn.Module (e.g. a CNN) and the rest of this function works
    unchanged — everything downstream of `ParamSpec.from_module` is
...
def sample_bnn_classification(
    ...
    """Sample from the posterior of a multiclass classification BNN.

    `samples` is a [n_resample, D] tensor of posterior draws. To compute
    class probabilities on new data:

        likelihood = target.meta["model"].likelihood
        logits = torch.stack([likelihood.predict(b, X_new) for b in samples])
        probs = torch.softmax(logits, dim=-1).mean(0)   # [N_new, n_classes]
    """
...
# ===========================================================================
# Demo: a small synthetic classification problem (three Gaussian blobs)
# ===========================================================================
