...
Runs four PDMP samplers, Gaussian-prior NUTS, and optionally horseshoe NUTS.
...
intercept slot still exists in the model and is sampled.

Usage:
    python -m sazz.scripts.logistic_regression

    python -m sazz.scripts.logistic_regression --N 2000 --D 200 \
        --n-signals 10 --signal-scale 2.0 --intercept 0 \
        --prior-std 5.0 --include-horseshoe --save
...
class Config:
    N: int = 300
    D: int = 9                          # covariates; total dim is D + 1 (intercept)
    n_signals: int = 3
    signal_scale: float = 1.5
    intercept_true: float = 0.5         # can be 0 — slot still exists in model
    prior_std: float = 1.0
    intercept_prior_std: float = 10.0   # must be strictly positive
    n_skel: int = 10_000
...
def run_pdmps(target, cfg: Config) -> list[dict]:
    # Intercept is coordinate 0 in the PDMP target (D + 1 wide)
...
# ===========================================================================
# NUTS — always samples an intercept; `prior` controls only the prior on β
# ===========================================================================
...
def main():
    ...
    p.add_argument("--out", type=Path, default=Path("results/logistic_regression"))
    ...
    # --- Data + held-out test (ones column always added) ---
    ...
    # --- Metrics (no closed form for logistic; NUTS-Gaussian is the σ-ratio reference) ---
