"""
FastGridStickyBoomerangSampler_Cheap -- a duplicate of
fast_grid_sticky_boomerang.py (FastGridStickyBoomerangSampler) that
ADDITIONALLY supports resuming sample() across separate calls. See
fast_grid_sticky_zigzag_cheap.py's module docstring for the full rationale
(staged runs to bound transient chunk_*.pt disk usage for very large N) and
resume-contract details -- identical here, with one extra piece of state:
dt_refresh (the exponential velocity-refresh clock's remaining time), which
ZigZag has no analog of. See this file's sample() docstring for the exact
resume_state keys.

This file exists as a full duplicate, not a parameter added to the
original, specifically so fast_grid_sticky_boomerang.py -- already used by
completed, working runs -- is never touched. See fast_cheap_mnist_cnn.py
for the staging wrapper that drives this.

self._grid_t_max and self.x_ref/self.Sigma (set by preprocess(), called
once before any sample() call) are NOT part of resume_state -- they live on
the sampler INSTANCE and already persist correctly across staged calls as
long as the SAME sampler object (already preprocess()-ed) is reused for
every stage.

---

Original module docstring (fast_grid_sticky_boomerang.py) below, describing
the sticky-Boomerang grid-bound design this file inherits unchanged except
for the resume additions in sample() itself:
...
class FastGridStickyBoomerangSampler_Cheap(GridBoomerangSampler):
    """
    ...
    def sample(
        ...
        """
        See fast_grid_sticky_zigzag_cheap.py's sample() docstring for the
        full chunk_size/chunk_dir/flush_diag_every/resume_state contract --
        identical here except resume_state additionally carries
        "dt_refresh" (remaining time to the next velocity-refresh event;
        ZigZag has no refresh clock, so its resume_state has no such key).
        The one Boomerang-specific wrinkle unrelated to resume: the refresh
        branch can write a SECOND skeleton row within the same while-loop
        iteration (after the regular bounce/freeze/thaw/no-event branch
        above it may have already written one) -- both routed through the
        same write_row() closure, whose flush check fires after every
        individual row write, not once per loop body, so this is handled
        correctly by construction.

        resume_state=<dict>: x0 is ignored when given (both cannot apply at
        once -- raises otherwise). _reset_sticky_state(), the fresh
        self.x_ref + self._refresh_velocity() cold-start draw, and
        _apply_cold_start() are all bypassed; frozen_mask/frozen_velocity/
        thaw_deadline/position/velocity/current_time/dt_refresh are restored
        from resume_state instead, exactly continuing the previous call's
        trajectory. self._grid_t_max/self.x_ref/self.Sigma are NOT part of
        resume_state (see module docstring -- must already be correct on
        this reused sampler instance).
        """
        ...
        if resume_state is not None:
            ...
            dt_refresh = float(resume_state["dt_refresh"])
        else:
        ...
        # resume_time_offset (0.0 on a cold start) -- see
        # fast_grid_sticky_zigzag_cheap.py's identical seeding for why this
        # must continue the previous stage's global simulation clock.
        ...
        # call -- see fast_grid_sticky_zigzag_cheap.py's identical block for
        # the non-chunking-dependent-correctness rationale. dt_refresh is
        # decremented every iteration (dt_refresh -= advance / -= tau) right
        # up to the loop's exit, so its value here IS the correct remaining
        # countdown from x_prev_state/v_prev_state/t_prev_holder's final
