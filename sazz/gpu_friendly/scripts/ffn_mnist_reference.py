"""
Standalone MAP + Sigma_inv (empirical Fisher) finder for the MNIST FFN-BNN
target -- the fully-connected counterpart to lenet_reference.py/
cnn_reference.py, for a plain FFN (Izmailov et al.-style MLP: two 256-unit
hidden layers on flattened 28x28 pixels) instead of a CNN/LeNet5. Kept
separate from utils/warmup.py::find_reference_bnn (the generic path every
other gpu_friendly script uses), same rationale as lenet_reference.py: this
script owns its own pipeline and is free to diverge for this target's scale.

Ported near-verbatim from lenet_reference.py -- run_map/eval_accuracy/
full_log_lik/empirical_fisher_diag_(vmapped|scaled) are IDENTICAL (they only
touch bm/beta, architecture-agnostic), and load_mnist is identical except
for one addition: FFN.forward expects a flat [N, 784] input, not a
[N, 1, 28, 28] image, so images are flattened right after loading. The only
other divergence from lenet_reference.py is the model itself (FFN instead
of LeNet5) and empirical_fisher_diag_vmapped's per-example unsqueeze shape
(flat [n, 1, 784], not LeNet5's [n, 1, 1, 28, 28]).

Usage:
    python -m sazz.gpu_friendly.scripts.ffn_mnist_reference --n-steps 2000
    python -m sazz.gpu_friendly.scripts.ffn_mnist_reference --full --batch-size 512 --n-steps 10_000
"""
...
# Izmailov et al.-style MLP: flattened 28x28 input, two 256-unit hidden
# layers, 10-way softmax output. Must match whatever a downstream sampling
# script (e.g. a future ffn_mnist_grid.py) builds its target with -- a
# checkpoint produced with one architecture/activation/prior scale and
# loaded into a mismatched target is a silent-garbage bug, not a mere
# inefficiency (see lenet_reference.py's identical comment).
...
# ===========================================================================
# Data -- identical to lenet_reference.py::load_mnist, plus a flatten step
# (FFN.forward wants [N, 784], not [N, 1, 28, 28]).
# ===========================================================================
...
def load_mnist(n: Optional[int], train_frac: float, seed: int, data_dir: Path,
    ...
    Every X tensor is flattened to [N, 784] here (torch.flatten(x, 1)) --
    the one addition over lenet_reference.py's identical loader, since FFN
    (unlike LeNet5) has no internal reshape and expects a flat vector.
    """
...
# ===========================================================================
# MAP -- Adam, module-parameter init (NOT torch.randn). Identical to
# lenet_reference.py::run_map -- architecture-agnostic (only touches
# bm/beta), reproduced here rather than imported so this script keeps its
# own standalone pipeline (same convention lenet_reference.py itself uses
# relative to cnn_reference.py's run_map).
# ===========================================================================
...
# ===========================================================================
# Sigma_inv -- vmapped empirical Fisher diagonal. Identical math to
# lenet_reference.py's version; only the per-example unsqueeze shape
# differs (flat [n, 1, 784] here, vs LeNet5's [n, 1, 1, 28, 28]) since X is
# already flattened by load_mnist above.
# ===========================================================================
...
def empirical_fisher_diag_vmapped(bm: BayesianModule, x_ref: Tensor, n_batch: int) -> Tensor:
    ...
    X_batch = X[idx].unsqueeze(1)  # [n, 1, 784]
    y_batch = y[idx].unsqueeze(1)  # [n, 1]
...
def main():
    ...
    parser.add_argument("--batch-size", type=int, default=None,
                         help="Minibatch size for the MAP step. Default (None) "
                              "is full-batch gradients. When set and < n_train, "
                              "uses the standard unbiased-subsampling rescale.")
    ...
    print("=" * 72)
    print(f"FFN MNIST reference (MAP + vmapped Fisher)  |  pool={'full (60000)' if n is None else n}  "
    ...
    print(f"  D = {bm.D}  layer_sizes={LAYER_SIZES}  activation={ACTIVATION}  "
    ...
    name = args.name or f"ffn_mnist_reference_N{pool_size}_steps{args.n_steps}"
