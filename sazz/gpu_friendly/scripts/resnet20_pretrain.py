...
# Data augmentation (random crop/flip) is deliberately DEFERRED here -- adds
# real pipeline complexity (per-batch random transforms) this repo's
# existing scripts don't have (they all use fixed, pre-materialized
# tensors); revisit only if pretrain accuracy is unacceptably poor without it.
# ===========================================================================
...
def run_pretrain(module: nn.Module, data: dict[str, Any], n_epochs: int, batch_size: int,
                  lr: float, momentum: float, weight_decay: float, log_every: int) -> dict:
    module.train()
