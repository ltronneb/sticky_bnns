"""
Standalone MAP + Sigma_inv (empirical Fisher) finder for the MNIST
LeNet5-BNN target -- the LeNet5 counterpart to cnn_reference.py, kept
separate from utils/warmup.py::find_reference_bnn (the generic path
every other gpu_friendly script uses) so that path stays untouched --
...
Two deliberate divergences from find_reference_bnn, both load-bearing:
  ...
  - The empirical-Fisher diagonal is computed with ONE
    ...
    LeNet5.forward's torch.flatten(x,1) then produces the wrong shape
    (confirmed by direct repro during planning, originally for CNN --
    same reasoning applies here since LeNet5 also takes (1,28,28) images).

Usage:
    python -m sazz.gpu_friendly.scripts.lenet_reference --n-steps 2000
    python -m sazz.gpu_friendly.scripts.lenet_reference --n 5000 --n-steps 2000
    python -m sazz.gpu_friendly.scripts.lenet_reference --full --batch-size 512 --n-steps 10_000
"""
...
def main():
    ...
    print("=" * 72)
    print(f"LeNet5 reference (MAP + vmapped Fisher)  |  pool={'full (60000)' if n is None else n}  "
    ...
    print("\n[model]")
    module = LeNet5(activation=ACTIVATION, pool=POOL)
    ...
    name = args.name or f"lenet_reference_N{pool_size}_steps{args.n_steps}"
