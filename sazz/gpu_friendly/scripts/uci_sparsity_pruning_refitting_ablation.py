...
Sibling script uci_sparsity_ablation.py covers the OTHER side of this
comparison -- fits the same MAP but skips prune/refit entirely and always
samples with cold_start_threshold=None (the unpruned-MAP baseline), so
final sparsity there is earned live via kappa alone, with nothing inherited
from a cold start. That script's --prior-inclusion-weight sweep is the
canonical baseline to compare this pruned+refit mode against.

Usage:
    python -m sazz.gpu_friendly.scripts.uci_sparsity_pruning_refitting_ablation \\
...
def run_split(dataset_name: str, split_id: int, data: dict[str, Any], cfg: BNNConfig,
    ...
    sd = out_dir / dataset_name / f"split_{split_id:02d}" / "pruned_MAP" / f"piw_{cfg.prior_inclusion_weight:g}"
...
def main():
    ...
    parser.add_argument("--prior-inclusion-weight", nargs="+", type=float, default=[0.1],
                         help="Sticky-only spike-and-slab prior inclusion probability w, fed to "
                              ...
                              "Accepts multiple values to sweep in one invocation, e.g. "
                              "--prior-inclusion-weight 0.1 0.3 0.5 0.7 -- each weight's sampler "
                              "output goes to its own <out>/<dataset>/split_<k>/piw_<w>/ "
                              "subdirectory (the shared pruned+refit MAP checkpoint under "
                              "<out-maps>/ is fit once and reused across all weights, since "
                              "pruning happens before kappa is built). Default here is [0.1] "
                              "(NOT configs_for's usual 0.7 default) -- smoke-tested on "
                              "boston/small: w=0.7 drains the cold-started ~72%% sparsity down to "
                              "36-47%% within 500 events (kappa too large, thaw-dominated); w=0.1 "
                              "holds ~72%% (zigzag, roughly balanced freeze/thaw) or grows to ~77%% "
                              "(boomerang, freeze-dominated). w=0.7 would defeat the point of "
                              "cold-starting from a pruned MAP.")
    ...
    print(f"\nRunning {datasets_to_run} | splits: {args.splits} | "
          ...
          f"prune_rmse_tolerance={args.prune_rmse_tolerance} | "
          f"prior_inclusion_weight sweep={args.prior_inclusion_weight} | "
          f"N_SKELETON={args.n_skeleton}")

    # The pruned+refit MAP checkpoint doesn't depend on prior_inclusion_weight
    # (pruning happens before kappa is built), so it's fit once per split on
    # the first weight and reused (via load_pruned_checkpoint) for the rest --
    # only --refit-maps forces every weight to refit its own MAP.
