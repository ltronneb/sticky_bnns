...
N_SKELETON = 50
N_RESAMPLE = N_SKELETON   # NOT a large fixed constant -- resample_*_torch
                            # materializes [N_resample, D] arrays; at CNN
                            # scale (D~19466) a UCI-scale N_RESAMPLE (50k)
                            # would be ~3.9GB PER ARRAY, and resampling far
                            # more points than the skeleton contains is
                            # statistically meaningless regardless.
N_SAVE = N_RESAMPLE         # no NUTS baseline here, so no NUTS_DRAWS*NUTS_CHAINS
                            # analog to borrow (unlike uci_bnn_grid.py) --
                            # this script needs its own explicit value.
BURNIN_FRAC = 0.2
...
def run_grid_sticky_zigzag(dataset_name: str, split_id: int, data: dict[str, Any], cfg: CNNConfig,
    ...
    result = sampler.sample(N=N_SKELETON, x0=x_pruned, diagnostics=True)
...
def main():
    global N_SKELETON, N_RESAMPLE, N_SAVE
