"""
Standalone pure-SGD FFN trainer for the MNIST target -- the frequentist
counterpart to ffn_mnist_reference.py, mirroring lenet_sgd.py for the FFN
architecture instead of LeNet5.

ffn_mnist_reference.py fits a MAP point (Adam, full-batch, Gaussian
log-prior in the loss) plus an empirical-Fisher Sigma_inv, for use as a
Boomerang reference measure. THIS script instead fits a plain SGD-with-
momentum classifier: minibatches, cross-entropy only, no log-prior term, no
Sigma_inv. It exists so analyze_ffn_mnist_paper.py can plot a genuine
frequentist baseline (Izmailov et al. 2021, Fig 15, use SGD in exactly this
role) alongside the sticky posteriors and the MAP point -- and critically,
one that is NOT the pruned+refit x_ref MAP checkpoint those posteriors are
compared against, so the noise-robustness comparison has an SGD curve that
is genuinely unrelated to that MAP.

Deliberately kept separate from ffn_mnist_reference.py rather than bolted on
as a flag, same rationale as lenet_sgd.py vs lenet_reference.py: almost
nothing is shared between a MAP fit and a pure SGD fit. The only things held
in common (and copied, not imported, per this tree's per-script-local-loader
...
Usage:
    python -m sazz.gpu_friendly.scripts.ffn_sgd --epochs 30
    python -m sazz.gpu_friendly.scripts.ffn_sgd --full --epochs 40 --batch-size 128
    python -m sazz.gpu_friendly.scripts.ffn_sgd --n 10000 --epochs 50 --lr 0.05
"""
...
# Same architecture constants as ffn_mnist_reference.py -- a checkpoint
# trained with one architecture/activation and loaded into a target built
# with another is a silent-garbage bug. The SGD baseline must match the
# samplers' FFN exactly for the pixel-noise comparison to be meaningful.
...
# ===========================================================================
# Data -- identical to ffn_mnist_reference.py::load_mnist (flattened input).
# ===========================================================================
...
def load_mnist(n: Optional[int], train_frac: float, seed: int, data_dir: Path,
    ...
    ffn_mnist_reference.py::load_mnist so the SGD baseline sees the same
    data as the MAP, including the [N, 784] flatten FFN.forward expects.
    """
...
@torch.no_grad()
def evaluate(module: FFN, X: Tensor, y: Tensor, chunk: int = 2048) -> tuple[float, float]:
...
def train_sgd(module: FFN, data: dict[str, Any], epochs: int, lr: float,
...
def main():
    ...
    print("=" * 72)
    print(f"FFN pure SGD baseline  |  pool={'full (60000)' if n is None else n}  "
    ...
    print("\n[model]")
    module = FFN(LAYER_SIZES, activation=ACTIVATION).to(dtype=DTYPE, device=DEVICE)
    D = sum(p.numel() for p in module.parameters())
    print(f"  D = {D}  layer_sizes={LAYER_SIZES}  activation={ACTIVATION}")
    ...
    name = args.name or f"ffn_sgd_N{pool_size}_epochs{args.epochs}"
