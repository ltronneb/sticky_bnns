...
from .priors_torch import GaussianPrior
...
def make_linear_regression(X, y, prior_std=1.0, intercept_prior_std=10.0,
...
def make_logistic_regression(X, y, prior_std=1.0, intercept_prior_std=10.0,
                           noise_std=1.0, diagonal_only=True, adam_steps=1000, adam_lr=1e-2,
                           dtype=torch.float64, device="cpu"):
    X = X.to(dtype=dtype, device=device)
    y = y.to(dtype=dtype, device=device)
